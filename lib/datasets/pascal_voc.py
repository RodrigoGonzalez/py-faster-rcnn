...
class pascal_voc(imdb):
    def __init__(self, image_set, year, devkit_path=None):
        imdb.__init__(self, f'voc_{year}_{image_set}')
        self._year = year
    ...
    def _get_default_path(self):
        ...
        """
        return os.path.join(cfg.DATA_DIR, f'VOCdevkit{self._year}')
    ...
    def rpn_roidb(self):
        if int(self._year) != 2007 and self._image_set == 'test':
            return self._load_rpn_roidb(None)

        gt_roidb = self.gt_roidb()
        rpn_roidb = self._load_rpn_roidb(gt_roidb)
        return imdb.merge_roidbs(gt_roidb, rpn_roidb)
    ...
    def _load_selective_search_roidb(self, gt_roidb):
        filename = os.path.abspath(
            os.path.join(cfg.DATA_DIR, 'selective_search_data', f'{self.name}.mat')
        )
        assert os.path.exists(
            filename
    ...
    def _get_voc_results_file_template(self):
        ...
        filename = f'{self._get_comp_id()}_det_{self._image_set}' + '_{:s}.txt'
        path = os.path.join(
            self._devkit_path, 'results', f'VOC{self._year}', 'Main', filename
        )
        return path
